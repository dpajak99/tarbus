...
public class DataHelper {
  public static List<Vehicle> parseJsonToVehicle(LiveJsonVehicles liveJsonVehicles) {

    List<Vehicle> vehicles = new ArrayList<>();
    for (int i = 0; i < liveJsonVehicles.getJsonVehicles().size(); i++) {
      try {
        JSONArray jsonArray = new JSONArray(liveJsonVehicles.getJsonVehicles().get(i).getContent());
  ...
  public static List<LiveBusStop> parseRouteBusStops(JsonArray array) {
    List<LiveBusStop> busStops = new ArrayList<>();
    for (int i = 0; i < array.size(); i++) {
      JsonArray busArray = array.get(i).getAsJsonArray();
      LiveBusStop busStop = new LiveBusStop(
  ...
  public static List<HolderRoute> parsePolylines(JsonArray array) {
    List<HolderRoute> routeHolders = new ArrayList<>();
    double tmp = 0;
    ...
    for (int i = 0; i < array.size(); i++) {
      ...
      routeHolders.add(new HolderRoute(routeElement.get(0).getAsInt(), routeElement.get(1).getAsInt(), routeElement.get(2).getAsInt(), routePoints));
    }
